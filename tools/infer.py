...
def parse_args():
    parser = argparse.ArgumentParser(description='detection on great ape video footage ')
    parser.add_argument('--input', help='the input dir of videos or single video path')
    parser.add_argument('--config', help='the model config file path (default: Retinanet based res50-tcm-scm model)')
    parser.add_argument('--checkpoint', help='checkpoint of the model (default:Retinanet based res50-tcm-scm model)')
    parser.add_argument('--output_dir', default='./output',help='the output video dir')
...
def main():
    ...
    checkpoint=args.checkpoint #= './workdir/chimp/retinanet_r50/tc_mode2_train7_test21/epoch_7.pth'
    config= args.config #'configs/chimp/chimptest.py'
    if osp.isfile(args.input):
        videos=[args.input]
    else:
        videos=os.listdir(args.input)
        videos=[osp.join(args.input,v)for v in videos]
    ...
    for video in tqdm.tqdm(videos):
        ...
        video_metas.append({'height':video_frames.height,'width':video_frames.width,'fps':video_frames.fps})
        frame_path=os.path.join(args.tmpdir,'original/',os.path.basename(video))
    ...
    for m in range(len(precess_videos)):
        ...
        if precess_video in os.listdir(args.output_dir):
            continue
        dataset=datasets[m]
        for i, data in tqdm.tqdm(enumerate(data_loader)):
            with torch.no_grad():
                result = model(return_loss=False, rescale=True, **data)
            show_result(i,data,result,cfg.img_norm_cfg,dataset,outfile_dir=os.path.join(args.tmpdir,'processed/',video_basename))
        # after process each frame, convert them to video stream
        dist.barrier()
        rank,_=get_dist_info()
        if rank==0:
            frame2videos(os.path.join(args.tmpdir,'processed/',video_basename),osp.join(args.output_dir,video_basename+'.mp4'),video_meta)
        dist.barrier()
