...
def main():
    ...
    if args.test==1:
        ...
        model = build_detector(cfg.model, train_cfg=None, test_cfg=cfg.test_cfg,video=video_model)

        # load 2 or 1 checkpoint here
        checkpoints=get_eval_pth(cfg.work_dir)
        for checkpoint in checkpoints:
            load_checkpoint(model, osp.join(cfg.work_dir,checkpoint), map_location='cpu')
            ...
            if not distributed:
                ...
                outputs = multi_gpu_test(model, data_loader, osp.join(cfg.work_dir,'temp__'))

            rank, _ = get_dist_info()
            dist.barrier()
            if rank ==0:
                ep = checkpoint.split('.')[0].split('_')[-1]
                raw_result=osp.join(cfg.work_dir,'raw_result_final_ep{}.txt'.format(ep))
                vid_result2txt(outputs,raw_result)
                #save the result
                outputs=[result['result'] for result in outputs]
                final_evaluate(outputs, dataset,int(ep),cfg.work_dir,refresh=True)
            dist.barrier()
        #last thing is to copy config file:args.config into workdir: cfg.work_dir
        if rank==0:
