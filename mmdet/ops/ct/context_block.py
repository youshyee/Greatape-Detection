...
#             x=self.model(x)
#             return  x
#     tc_short=TemporalContextBlock(13,7,
#                     repeat_mode=True,
#                     is_position_encoding= False,
#                     window_size=9,
#                     detach=False,
...
#     tc_short.train()
#     optim = torch.optim.Adam(tc_short.parameters(), lr=1e-1)
#     for i in range(10):
#         inputs=torch.randn(21,13,5,7)#[b=3,s=7,c=13,h=5,w=7]
#         extractor = CamExtractor(tc_short)
#         out=extractor.forward_pass(inputs)
#         one_hot_output=torch.randn(out.size())
#         tc_short.zero_grad()
#         out.backward(gradient=one_hot_output, retain_graph=True)
#         grad=extractor.gradients.detach().cpu().numpy()
#         grad=grad.reshape(3,7,13,5,7)
#         # input_grad=inputs.grad.reshape(3,7,13,5,7)
#         # print(input_grad[:,6,...].nonzero())
#         # inputs.grad.zero_()
#         print(grad[:,5,...].nonzero())
#         optim.step()
